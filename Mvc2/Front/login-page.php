...
if (isset($_POST['login'])) {
    ...
    $rowf = mysqli_fetch_assoc($query); //Fetch row data in associative array from database.   
    $_SESSION['email'] = $rowf['email_id'];
    $_SESSION['firstname'] = $rowf['first_name'];
    $_SESSION['lastname'] = $rowf['last_name'];
    ...
    if (!empty($email) && !empty($password)) {
        ...
        if ($email_count) {
            $db_pass = $rowf['password']; //fetch password from database.

            if ($rowf['is_email_verified'] != 1) {  //is email verified checking.
                ...
                <div class="col-md-6 col-sm-6 col-xs-6">
                    ...
                    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="post" id="loginForm" class="all-form wow zoomIn">
                        ...
                        <div class="form-group <?php if ($email_error) {
                            ...
                            <input type="email" name="email" value="<?php if (isset($_COOKIE['emailcoockie'])) {
                                                                        echo $_COOKIE['emailcoockie'];
                                                                    } ?>" placeholder="Enter your email adress">
                            <small style="color:red;">Incorrect Email</small>
                        </div>

                        <div class="form-group <?php if ($pass_error) {
                                                    echo 'error';
                                                } ?>">
                            <label for="password">Password <a id="fgt-pwd" href="forgotPassword.php">Forgot Password?</a></label>
                            <input type="password" name="password" id="password" value="<?php if (isset($_COOKIE['passcoockie'])) {
                                                                                            echo $_COOKIE['passcoockie'];
